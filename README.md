# 🎓 Data Professional Certification Portfolio
### Wisang Residata, M.T. 

This repository serves as a centralized hub for my professional certification history across the three pillars of the data landscape: **Data Analysis, Data Science, and Data Engineering**. 

With a background in **Industrial Engineering (Universitas Indonesia & ITATS)** and **Statistics (ITS)**, I leverage these certifications to bridge the gap between academic theory and industry-leading technical execution.

---

## 🏗️ The Three Pillars

### 1. Data Engineering 🛠️
*Focusing on scalable architectures, CI/CD pipelines, and robust data movement.*
* **Data Engineering Zoomcamp** | DataTalksClub | *In Progress*
  * Key Tech: Docker, PostgreSQL, Terraform, Apache Spark, Airflow, Kestra, dlt.

### 2. Data Science & Machine Learning 🤖
*Focusing on predictive modeling, MLOps, and statistical inference.*
* **Associate AI & ML Engineer** | BootUp Academy.
* **Core Designer Certificate** | Dataiku.
* **Specialization in Inferential Statistics**

### 3. Data Analytics 📊
*Focusing on business intelligence, visualization, and actionable insights.*
* **Tableau Specialist** | (Internal/Project Based - Bank BRI).
* **Power BI Ecosystem Architect** | (Kejaksaan Tinggi RI Project).

---

## 🎓 Academic Foundation
* **Master of Technology (M.T.) in Industrial Engineering** | Universitas Indonesia.
* **Bachelor of Engineering (S.T.) in Industrial Engineering** | ITATS.
* **Degree in Statistics** | Institut Teknologi Sepuluh Nopember (ITS).

---

## 📈 Professional Impact
My certification path is driven by real-world challenges encountered during my tenure as:
* **Tenaga Ahli Data** at DPR-RI.
* **Big Data Intelligence Consultant** at Kejaksaan Tinggi RI.
* **Product Analyst Consultant** at ASTRA Honda Motor.

---

## 📫 Connect with Me
* **LinkedIn:** [residata](https://linkedin.com/in/residata)
* **GitHub:** [sangresidata](https://github.com/sangresidata)
* **Email:** wisangresidata@gmail.com

---
*"Turning complex data into narrative insights."*
